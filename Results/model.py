...
# Load local medical dictionary for RAG (Retrieval-Augmented Generation)
try:
    with open("data/medical_dict.json", "r", encoding="utf-8") as f:
        LOCAL_MED_DICT = json.load(f).get("medical_entities", {})
    print(f"Loaded {len(LOCAL_MED_DICT)} terms from local dictionary.")
except Exception as e:
    print(f"Warning: Could not load local dictionary: {e}")
    LOCAL_MED_DICT = {}
    ...
    for i, chunk in enumerate(chunks):
        ...
        try:
            ...
            for ent in entities:
                ...
                # Skip sub-word tokens or incredibly short noisy strings
                if len(word) > 2 and not word.startswith("#"):
                    terms.add(word)
        except Exception as e:
            print(f"Warning: NER skipped a chunk due to an error: {e}")
            
    # Enrich the extracted terms by finding their definitions in our local dictionary!
    enriched_terms = []
    for term in terms:
        # Check if we have a simplified definition stored locally
        definition = LOCAL_MED_DICT.get(term)
        if definition:
            enriched_terms.append(f"{term} (Definition: {definition})")
        else:
            enriched_terms.append(term)
            
    print(f"Found {len(terms)} unique medical terms, {len([t for t in terms if t in LOCAL_MED_DICT])} matched in local dictionary.")
...
3. **Explanation of Medical Terms**: Briefly define the key medical terms found in the report. (Rely strictly on the dictionary definitions provided in the list above where available).
